...
async function start(client) {
  client.onMessage(async (message) => {
    const formattedMessage = message.body.toLowerCase().trim(); // Format the message to lowercase and remove extra spaces
...
async function sendRawSignal(to, client) {
  const rawResponse = await axios.get(`${process.env.DEVICE_URL}/SENDRAW?P1=${process.env.RAW_SIGNAL}`);
...
async function sendFileSignal(to, client) {
  ...
  const fileResponse = await axios.get(`${process.env.DEVICE_URL}/APPLY?P1=CURRENT&P2=vale_do_cedro-gate.sub`);
